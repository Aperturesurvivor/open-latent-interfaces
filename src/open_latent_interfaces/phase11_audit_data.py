...
PHASE11_AUDIT_TEMPLATES = (
    (
        "Add the two recorded amounts, {a} and {b}. Reply solely with "
        "Answer=<integer>."
    ),
    (
        "What integer results when {a} is combined with {b}? Output exactly "
        "Answer=<integer>."
    ),
    (
        "Two counters show {a} and {b}. State their total and no explanation: "
        "Answer=<integer>."
    ),
)
...
PHASE9E_PARAMETERS = {"seed": 20261205}


@dataclass(frozen=True)
class Phase11AuditExample:
    example_id: str
    template_family: str
    operand_a: int
    operand_b: int
...
def _prior_dataset_hash_items() -> tuple[tuple[str, str], ...]:
    ...
    phase9e = build_phase9e_audit(**PHASE9E_PARAMETERS)
    return tuple(
        {
            ...
            "phase9e": phase9e_audit_sha256(phase9e),
        }.items()
    )
...
def _prior_canonical_pairs() -> frozenset[tuple[int, int]]:
    sources = (
        ...
        build_phase9e_audit(**PHASE9E_PARAMETERS),
    )
...
def build_phase11_audit(
    *,
    seed: int = 20261305,
) -> list[Phase11AuditExample]:
    ...
    for leading_digit in range(1, 10):
        feasible_tens = [
            ...
            if (tens + 7 * leading_digit) % 10 != 9
        ]
        rotation = (2 * leading_digit) % len(feasible_tens)
        rotated = feasible_tens[rotation:] + feasible_tens[:rotation]
        carry_tens = set(rotated[:5])
        for tens_digit in range(10):
            ones_digit = (tens_digit + 7 * leading_digit) % 10
            ...
            if pair is None:
                raise ValueError(
                    "could not construct fresh Phase 11 audit pair for "
            ...
            template_index = (2 * leading_digit + tens_digit) % len(
                PHASE11_AUDIT_TEMPLATES
            )
            examples.append(
                Phase11AuditExample(
                    example_id=(
                        f"phase11-audit-l{leading_digit}-"
                        f"t{tens_digit}-o{ones_digit}"
                    ),
                    template_family=f"phase11-audit-{template_index}",
                    operand_a=a,
                    operand_b=b,
                    result=result,
                    ...
                    prompt=PHASE11_AUDIT_TEMPLATES[template_index].format(
                        a=a,
                        b=b,
                    ),
                )
    ...
    assert_phase11_audit_invariants(examples)
    return examples


def assert_phase11_audit_invariants(
    examples: list[Phase11AuditExample],
) -> None:
    if len(examples) != 90:
        raise ValueError("Phase 11 audit must contain exactly 90 examples")
    if len({row.example_id for row in examples}) != len(examples):
        raise ValueError("Phase 11 example IDs must be unique")
    if len({row.prompt for row in examples}) != len(examples):
        raise ValueError("Phase 11 prompts must be unique")
    canonical = _canonical_pairs(examples)
    if len(canonical) != len(examples):
        raise ValueError("Phase 11 canonical pairs must be unique")
    if canonical & prior_canonical_pairs():
        raise ValueError("Phase 11 pair overlaps prior data")
    for row in examples:
    ...
    expected_counts = {
        ...
        "template": (
            {f"phase11-audit-{index}": 30 for index in range(3)},
    ...
    for name, (expected, values) in expected_counts.items():
        ...
        if observed != expected:
            raise ValueError(
                f"Phase 11 {name} distribution mismatch: {observed}"
            )


def phase11_audit_sha256(examples: list[Phase11AuditExample]) -> str:
    encoded = json.dumps(
