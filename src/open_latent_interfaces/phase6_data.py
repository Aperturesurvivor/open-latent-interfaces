...
Phase6Split = Literal["fit", "selection", "development", "audit"]
...
PHASE6_TEMPLATES = {
    "fit": "Calculate {a} + {b}. Reply only with Answer=<integer>.",
    "selection": "What is the total of {a} and {b}? Reply as Answer=<integer>.",
    "development": "Add together {a} and {b}; output only Answer=<integer>.",
    "audit": (
        "A ledger lists {a} units in one entry and {b} in another. "
        "Give their combined amount only as Answer=<integer>."
    ),
}

# Phase 4 and Phase 5 used this exact shared corpus. Phase 6 excludes every
# canonical operand pair from it, including the exposed Phase 5 audit split.
...
def _reference_canonical_pairs() -> set[tuple[int, int]]:
    return {
        tuple(sorted((row.operand_a, row.operand_b)))
        for row in build_phase4_carry_quartets(**REFERENCE_PARAMETERS)
    }
...
def _quartet_rows(
    ...
    quartet_id = f"phase6-{split}-q{quartet_index:04d}"
    template = PHASE6_TEMPLATES[split]
    return [
        Phase6CarryExample(
...
def build_phase6_carry_quartets(
    *,
    seed: int = 20260901,
    ...
    if min(counts) < 1:
        raise ValueError("every Phase 6 split requires quartets")
    rng = random.Random(seed)
    candidates = _candidate_quartets()
    rng.shuffle(candidates)
    used = prior_canonical_pairs() | _reference_canonical_pairs()
    split_counts: tuple[tuple[Phase6Split, int], ...] = (
    ...
    examples: list[Phase6CarryExample] = []
    candidate_cursor = 0
    for split, count_per_digit in split_counts:
        ...
        while set(selected_by_digit.values()) != {count_per_digit}:
            if candidate_cursor >= len(candidates):
                raise ValueError("insufficient fresh carry quartets")
            ...
            quartet_index += 1
    assert_phase6_carry_invariants(examples)
    return examples


def assert_phase6_carry_invariants(
    examples: list[Phase6CarryExample],
) -> None:
    if len({example.example_id for example in examples}) != len(examples):
        raise ValueError("Phase 6 example IDs must be unique")
    historical = prior_canonical_pairs() | _reference_canonical_pairs()
    by_split: dict[str, set[tuple[int, int]]] = {}
    quartets: dict[str, list[Phase6CarryExample]] = {}
    for example in examples:
        canonical = tuple(sorted((example.operand_a, example.operand_b)))
        if canonical in historical:
            raise ValueError("Phase 6 pair overlaps historical data")
        if canonical in by_split.setdefault(example.split, set()):
            raise ValueError("Phase 6 pair repeats within a split")
    ...
    for index, left in enumerate(splits):
        for right in splits[index + 1 :]:
            if by_split[left] & by_split[right]:
                raise ValueError(f"Phase 6 pair leakage between {left} and {right}")
    ...
    for split in splits:
        ...
        if len(set(counts.values())) != 1:
            raise ValueError(f"carry quartets are not balanced: {split}")


def phase6_carry_sha256(examples: list[Phase6CarryExample]) -> str:
    encoded = json.dumps(
