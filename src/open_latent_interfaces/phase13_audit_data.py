...
from open_latent_interfaces.phase13_data import (
    prior_canonical_pairs as phase13_prior_canonical_pairs,
)
from open_latent_interfaces.phase13_data import (
    prior_dataset_hashes as phase13_prior_dataset_hashes,
)

PHASE13_AUDIT_TEMPLATES = (
    (
        "Find the exact total of ledger entries {a} and {b}. Emit only "
        "Answer=<integer>."
    ),
    (
        "An integer register contains {a}, then receives {b}. Report the "
        "final value as Answer=<integer>."
    ),
    (
        "Compute {a} plus {b} for this checksum. Return exactly "
        "Answer=<integer>."
    ),
)
...
def _prior_dataset_hash_items() -> tuple[tuple[str, str], ...]:
    hashes = phase13_prior_dataset_hashes()
    phase13 = build_phase13_examples()
    hashes["phase13"] = phase13_sha256(phase13)
...
def _prior_canonical_pairs() -> frozenset[tuple[int, int]]:
    phase13 = build_phase13_examples()
    return frozenset(
        phase13_prior_canonical_pairs() | _canonical_pairs(phase13)
    )
...
def build_phase13_audit(
    *,
    seed: int = 20261318,
) -> list[Phase13AuditExample]:
    ...
    for leading_digit in range(1, 10):
        feasible_tens = [
            ...
            if (tens + 5 * leading_digit + 8) % 10 != 9
        ]
        rotation = (4 * leading_digit + 1) % len(feasible_tens)
        rotated = feasible_tens[rotation:] + feasible_tens[:rotation]
        carry_tens = set(rotated[:5])
        for tens_digit in range(10):
            ones_digit = (tens_digit + 5 * leading_digit + 8) % 10
            ...
            if pair is None:
                raise ValueError(
                    "could not construct fresh Phase 13 audit pair for "
            ...
            template_index = (
                leading_digit + 2 * tens_digit
            ) % len(PHASE13_AUDIT_TEMPLATES)
            examples.append(
                Phase13AuditExample(
                    example_id=(
                        f"phase13-audit-l{leading_digit}-"
                        f"t{tens_digit}-o{ones_digit}"
                    ),
                    template_family=f"phase13-audit-{template_index}",
                    operand_a=a,
                    operand_b=b,
                    result=result,
                    ...
                    prompt=PHASE13_AUDIT_TEMPLATES[template_index].format(
                        a=a,
                        b=b,
                    ),
                )
    ...
    assert_phase13_audit_invariants(examples)
    return examples


def assert_phase13_audit_invariants(
    examples: list[Phase13AuditExample],
) -> None:
    if len(examples) != 90:
        raise ValueError("Phase 13 audit must contain exactly 90 examples")
    if len({row.example_id for row in examples}) != len(examples):
        raise ValueError("Phase 13 audit IDs must be unique")
    if len({row.prompt for row in examples}) != len(examples):
        raise ValueError("Phase 13 audit prompts must be unique")
    canonical = _canonical_pairs(examples)
    if len(canonical) != len(examples):
        raise ValueError("Phase 13 audit pairs must be unique")
    if canonical & prior_canonical_pairs():
        raise ValueError("Phase 13 audit pair overlaps prior data")
    for row in examples:
    ...
    expected_counts = {
        ...
        "template": (
            {f"phase13-audit-{index}": 30 for index in range(3)},
    ...
    for name, (expected, values) in expected_counts.items():
        ...
        if observed != expected:
            raise ValueError(
                f"Phase 13 audit {name} distribution mismatch: {observed}"
            )


def phase13_audit_sha256(examples: list[Phase13AuditExample]) -> str:
    encoded = json.dumps(
