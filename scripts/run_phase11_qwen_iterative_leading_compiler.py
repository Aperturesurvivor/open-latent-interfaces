#!/usr/bin/env python3
"""Select Qwen's bounded relinearization count for the leading compiler."""
...
def main() -> None:
    ...
    one_step_path = Path(config["one_step_result"])
    verify_sha256(dataset_path, config["dataset_config_sha256"])
    verify_sha256(one_step_path, config["one_step_result_sha256"])
    dataset_config = json.loads(dataset_path.read_text())
    one_step = json.loads(one_step_path.read_text())
    if dataset_config.get("audit_authorized", False):
        raise SystemExit("iterative selection requires a sealed audit")
    if one_step["selection"]["passes"]:
        raise SystemExit("iterative selection requires a non-passing one-step result")
    for field in ("hidden_state_index", "desired_margin", "norm_cap"):
        if one_step["selection"][field] != config[field]:
            raise SystemExit(f"iterative {field} does not match one-step fallback")
    ...
    report = {
        "schema_version": "oli.phase11-qwen-iterative-leading-compiler/v1",
        "created_at": datetime.now(UTC).isoformat(),
        "status": "selection_only",
        "model": model_config,
        "dataset_sha256": dataset_hash,
        "one_step_result_sha256": config["one_step_result_sha256"],
        ...
        "claim_boundary": (
            "Selection-only bounded iterative Qwen hidden-state compiler on "
            "exposed data. No integrated, audit, or model-general claim."
        ),
    }
